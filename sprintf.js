...
// NOTE: int64/uint64 is broken for large values. sprintf needs to be rewritten to handle int64 properly.

function sprintf_tokenize(fmt) {
	var out = [];
	var start = 0;
...
function vsprintf(fmt, args) { return sprintf_doit(sprintf_tokenize(fmt), args); }

function sprintf()
{
	var args = new Array(arguments.length - 1);
	for(var i = 0; i < args.length; ++i) args[i] = arguments[i+1];
	return sprintf_doit(sprintf_tokenize(arguments[0]), args);
}

function sprintf_is_digit(c)
{
    return c >= '0' && c <= '9';
}

function sprintf_skip_digits(fmt, index)
{
    while (sprintf_is_digit(fmt.charAt(index)))
    {
        index++;
    }
    return index;
...
function sprintf_x86_internal(cpu, fmt)
{
    var args = [];
    ...
    for (var i = 0; i < fmt.length; i++)
    {
        ...
        if (sprintf_is_digit(c))
        {
            i = sprintf_skip_digits(fmt, i);
        }
        else if (c == '*')
        {
            c = fmt.charAt(++i);
            args.push(va_arg(cpu, VA_ARG_I32));
        }
        ...
        switch (c)
        {
            ...
            case 's':
            case 'S':
                args.push(va_arg(cpu, VA_ARG_STRING));
                break;
            case 'n':
                // Not handling %n for now. We would need to write a new sprintf implementation to handle this properly.
                va_arg(cpu, VA_ARG_POINTER);
                args.push(null);
                break;
            case 'A':
            case 'a':
            ...
            case 'F':
            case 'f':
                args.push(va_arg(cpu, VA_ARG_DOUBLE));
                break;
            case 'd':
            case 'i':
                hasSigned = true;
        ...
        if (qualifier == 'l')
        {
            if (hasUnsigned)
            {
                args.push(va_arg(cpu, VA_ARG_U64));
            }
            else
            {
                args.push(va_arg(cpu, VA_ARG_I64));
            }
        }
        else if (hasSigned)
        {
        ...
        {
            args.push(va_arg(cpu, VA_ARG_U32));
        }
    }
...
function sprintf_x86(cpu, hasBufferSize, hasVAList)
{
    var espStart = cpu.get_esp();//cdecl
    
    var buffAddr = cpu.popU32();
    var buffSize = hasBufferSize ? cpu.popU32() : -1;
    var fmt = cpu.safe_read_string(cpu.popU32());
        
    if (hasVAList)
    {
        // Set esp to the va_list arg so that we can pop values (cdecl is used so esp is reset anyway)
        cpu.set_esp(cpu.popU32());
    }
    
    var result = sprintf_x86_internal(cpu, fmt);
    
    if (!cpu.isValidAddress(buffAddr, result.length))
    {
        throw "Out of bounds write on sprintf call at address " + buffAddr.toString(16);
    }
    
    if (hasBufferSize && result.length >= buffSize)
    {
        // Formatted string is too long for the output buffer
        return -1;
    }
    
    var offset = buffAddr - cpu.virtualMemoryAddress;
    for (var i = 0; i < result.length; i++)
    {
        cpu.virtualMemory[offset + i] = result.charCodeAt(i);
    }
    cpu.virtualMemory[offset + result.length] = 0;
    
    cpu.set_esp(espStart);
    cpu.set_eax(result.length);
}

function snprintf_x86(cpu)
{
    sprintf_x86(cpu, true, false);
}

function vsnprintf_x86(cpu)
{
    sprintf_x86(cpu, true, true);
}

function printf_x86(cpu)
{
    var espStart = cpu.get_esp();//cdecl
