...
FPU.prototype.dbg_log_fpu_op = function(op, imm8)
{
    ...
    if(imm8 >= 0xC0)
    {
        dbg_log(h(op, 2) + " " + h(imm8, 2) + "/" + (imm8 >> 3 & 7) + "/" + (imm8 & 7) +
                " @" + h(this.cpu.mem_eip >>> 0, 8) + " sp=" + this.stack_ptr + " st=" + h(this.stack_empty, 2), LOG_FPU);
    }
    else
    {
...
FPU.prototype.op_DF_reg = function(imm8)
{
    ...
    switch(mod)
    {
        case 4:
            ...
            {
                // fnstsw
                this.cpu.setReg16(reg_eax, this.load_status_word());//this.cpu.reg16[reg_ax] = this.load_status_word();
            }
            else
            {
                dbg_log(imm8);
